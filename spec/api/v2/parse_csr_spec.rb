  ...
  describe "Parse CSR response" do
    ...
    context "response is correct" do
      ...
      it "response is OK" do
        get_parse_csr("quickssl", @csr).response["response_code"].to_i.should equal(200)
      end

      it "body is correct"  do
        xml_response_parse_csr = File.read(File.join(Rails.root, 'spec','api','v2','patterns','parse_csr','response_parse_csr.xml'))
        get_parse_csr("quickssl", @csr).response_xml.should == xml_response_parse_csr
      end
